...
impl Interpreter {
    ...
    fn exec_input(&mut self, nodes: &Vec<AST>) -> Result<Execute> {
        ...
        if nodes[0].token.id == TID::STR {
            start = 1;
            self.print_stdout(format!("{}", &nodes[0].token).as_str());
        }
            ...
            match nodes[i].token.id {
                ...
                _ => {
                    ...
                    match self.eval(&nodes[i])? {
                        MV::Num(n) => self.print_stdout(format!(" {} ", n).as_str()),
                        MV::Str(ref s) => self.print_stdout(format!("{}", s).as_str()),
                    };
                }
            }
        }
